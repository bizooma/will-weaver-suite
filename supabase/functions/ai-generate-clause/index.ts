...
// AI clause generator. Auth required + 30 req/min per user.
serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }
  const guard = await guardAuthed(req, { maxBytes: 32_000, limit: 30, windowSeconds: 60 });
  ...
  try {
    ...
    const fieldLabel = field === "funeral_instructions" ? "Funeral & Burial Instructions" : field || "Clause";
    const userContext = typeof data === "string" ? data : JSON.stringify(data ?? {});
    ...
    } as const)[toneKey as "plain"|"formal"|"compassionate"|"concise"];
...
- Context (JSON): ${userContext.slice(0, 4000)}
...
- Do NOT include disclaimers or headings. Output only the clause text.`;
    ...
    const resp = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: "You are a careful estate planning assistant that drafts short, precise clauses in a respectful tone." },
